...
def fetch_medal_tally(df,year,country):
  ...
  if flag == 1:
    ...
    x = temp_df.groupby('region').sum()[['Gold','Silver','Bronze']].sort_values('Gold',ascending=
    False).reset_index()
...
def data_over_time(df,column):
    data_over_time_df = df.drop_duplicates(['Year', column]) \
        .groupby('Year')[column] \
        .nunique() \
        .reset_index()
    data_over_time_df.rename(columns={column: "count"}, inplace=True)
